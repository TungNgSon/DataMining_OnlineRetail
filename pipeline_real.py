"""
Data Mining Pipeline - Online Retail Dataset (UCI - THẬT)
...
def pick_dbscan_eps(X, min_pts=5, eps_grid=None):
    ...
    for eps in eps_grid:
        ...
        # Ràng buộc để kết quả DBSCAN có ý nghĩa trình bày
...
def label_kmeans_segments(stats_df):
    ...
    # Đảm bảo tên không trùng gây khó đọc biểu đồ
    used, normalized = {}, {}
...
# 0. CẤU HÌNH
# ─────────────────────────────────────────────
DATA_FILE = Path('Online_Retail.xlsx')
...
if not DATA_FILE.exists():
    candidates = sorted(Path('.').glob('Online_Retail*.xlsx'))
    if not candidates:
        candidates = sorted(Path('.').glob('Online Retail*.xlsx'))
    if len(candidates) == 1:
        DATA_FILE = candidates[0]

if not DATA_FILE.exists():
    raise FileNotFoundError(
        f"Không tìm thấy dữ liệu: {DATA_FILE.resolve()}\n"
...
# ─────────────────────────────────────────────
# 1. TIỀN XỬ LÝ (theo tài liệu)
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("BƯỚC 2: TIỀN XỬ LÝ DỮ LIỆU")
print("=" * 60)

# Bước 1: Xóa hàng thiếu CustomerID và Description (theo tài liệu)
before = len(df)
df.dropna(subset=['CustomerID', 'Description'], inplace=True)
print(f"Xóa hàng null (CustomerID/Description): {before:,} → {len(df):,} hàng")

# Bước 2: Loại invoice bị hủy (InvoiceNo bắt đầu bằng 'C')
before = len(df)
df = df[~df['InvoiceNo'].astype(str).str.startswith('C')]
print(f"Loại invoice hủy (bắt đầu 'C')       : {before:,} → {len(df):,} hàng")

# Bước 3: Loại Quantity âm và UnitPrice <= 0 (theo tài liệu)
before = len(df)
...
# ─────────────────────────────────────────────
# 2. FEATURE ENGINEERING: RFM (theo tài liệu)
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
...
# Ngày mốc = ngày cuối cùng có giao dịch + 1 (theo tài liệu)
...
# Frequency: số hóa đơn duy nhất (theo tài liệu: drop_duplicates InvoiceNo+CustomerID)
...
# ─────────────────────────────────────────────
# 3. XỬ LÝ NGOẠI LAI bằng IQR (theo tài liệu)
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
...
# ─────────────────────────────────────────────
# 4. CHUẨN HÓA (StandardScaler - theo tài liệu)
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("BƯỚC 5: CHUẨN HÓA DỮ LIỆU (StandardScaler)")
print("=" * 60)

# Giảm lệch phải cho Frequency và Monetary trước khi chuẩn hóa
...
# K-distance graph để tham khảo ngưỡng epsilon
...
# Chọn epsilon theo chất lượng phân cụm thay vì tune tay
...
# Plot 4: Scatter K-Means
ax4 = fig.add_subplot(gs[1, :2])
for i, lbl in enumerate(sorted(rfm_clean['KM_Label'].unique())):
    m = rfm_clean['KM_Label'] == lbl
    ax4.scatter(rfm_clean[m]['Recency'], rfm_clean[m]['Monetary'],
                c=COLORS_KM[i % 3], s=25, alpha=0.55, label=lbl, edgecolors='none')
ax4.set_title('K-Means: Recency vs Monetary', **TITLE_KW)
ax4.set_xlabel('Recency (ngày)', **LAB_KW)
ax4.set_ylabel('Monetary (£)', **LAB_KW)
...
# Plot 6: Heatmap z-score RFM theo K-Means segment
ax6 = fig.add_subplot(gs[2, :2])
hm = ax6.imshow(rfm_z.values, cmap='RdYlGn_r', aspect='auto', vmin=-2.5, vmax=2.5)
ax6.set_yticks(np.arange(len(rfm_z.index)))
ax6.set_yticklabels(rfm_z.index, fontsize=9)
ax6.set_xticks(np.arange(3))
ax6.set_xticklabels(['Recency', 'Frequency', 'Monetary'], fontsize=10)
ax6.set_title('K-Means Segment Profile (z-score)', **TITLE_KW)
for i in range(rfm_z.shape[0]):
    for j in range(rfm_z.shape[1]):
        ax6.text(j, i, f"{rfm_z.iloc[i, j]:.2f}", ha='center', va='center', fontsize=8, color='#1F1F1F')
cbar = fig.colorbar(hm, ax=ax6, fraction=0.046, pad=0.04)
cbar.ax.tick_params(labelsize=8)
ax6.set_facecolor('white')

# Plot 7: Customer share vs Revenue share
ax7 = fig.add_subplot(gs[2, 2])
x = np.arange(len(km_profile.index))
bw = 0.38
ax7.bar(x - bw/2, km_profile['CustomerSharePct'], width=bw, color='#1D9E75', alpha=0.9, label='Customer %')
ax7.bar(x + bw/2, km_profile['RevenueSharePct'], width=bw, color='#D85A30', alpha=0.9, label='Revenue %')
ax7.set_xticks(x)
ax7.set_xticklabels(km_profile.index, rotation=30, ha='right', fontsize=8)
ax7.set_ylabel('Tỷ trọng (%)', **LAB_KW)
ax7.set_title('K-Means: Customer vs Revenue Share', **TITLE_KW)
ax7.legend(fontsize=8)
ax7.set_facecolor('white')

# Plot 8: Scatter DBSCAN
ax8 = fig.add_subplot(gs[3, :2])
noise_m = rfm_clean['Cluster_DB'] == -1
ax8.scatter(rfm_clean[noise_m]['Recency'], rfm_clean[noise_m]['Monetary'],
...
for cid in sorted(rfm_clean[~noise_m]['Cluster_DB'].unique()):
    ...
    ax8.scatter(rfm_clean[m]['Recency'], rfm_clean[m]['Monetary'],
                c=COLORS_DB[cid % len(COLORS_DB)], s=25, alpha=0.55,
                label=f'Cluster {cid}', edgecolors='none')
ax8.set_title('DBSCAN: Recency vs Monetary\n(dấu ✕ đỏ = noise/outlier DBSCAN tự phát hiện)', **TITLE_KW)
ax8.set_xlabel('Recency (ngày)', **LAB_KW)
ax8.set_ylabel('Monetary (£)', **LAB_KW)
ax8.legend(fontsize=9); ax8.set_facecolor('white')
...
ax9.pie(db_c, labels=p_lbl, colors=p_col,
        autopct='%1.1f%%', startangle=90, textprops={'fontsize':8})
ax9.set_title('DBSCAN: Cluster + Noise', **TITLE_KW)

# Plot 10: So sánh metrics
...
for i, (metric, vals) in enumerate(metrics.items()):
    for j, val in enumerate(vals):
        ...
        ax10.bar(i*3 + j*1.1, val, 0.95,
                label=lbl if i == 0 else '_',
                color=bar_c[j], alpha=0.85)
        ax10.text(i*3 + j*1.1, val + 0.005, f'{val:.3f}',
                 ha='center', va='bottom', fontsize=10, fontweight='bold', color='#2C2C2A')
ax10.set_xticks([0.55, 3.55])
ax10.set_xticklabels(list(metrics.keys()), fontsize=11)
ax10.set_ylabel('Giá trị', **LAB_KW)
ax10.set_title('So sánh độ đo: K-Means vs DBSCAN', **TITLE_KW)
ax10.legend(fontsize=10); ax10.set_facecolor('white')

# Plot 11: Action table
...
tbl.set_fontsize(9)
tbl.scale(1, 1.4)
ax11.set_title('Actionable Insight theo Segment (K-Means)', **TITLE_KW)

fig.suptitle('Data Mining – Online Retail UCI (THẬT)\nCustomer Segmentation: K-Means vs DBSCAN (RFM Analysis)',
             fontsize=15, fontweight='bold', color='#2C2C2A', y=0.995)

plt.savefig(OUTPUT_DIR / 'pipeline_real_results.png',
            dpi=150, bbox_inches='tight', facecolor='#F8F8F6')
plt.close()
print("\n✓ Biểu đồ đã lưu.")

# Xuất profile để báo cáo
...
# Xuất dữ liệu chi tiết để web app vẽ tương tác
...
# Xuất profile DBSCAN để web app hiển thị/lọc
db_profile = (
...
# đổi tên index -1 thành 'Noise' cho dễ đọc
...
# ─────────────────────────────────────────────
print("\n" + "=" * 60)
print("BƯỚC 8: INSIGHT PHÂN KHÚC KHÁCH HÀNG")
print("=" * 60)
...
print("\nKết quả đầu ra:")
print(f"  - Ảnh dashboard: {(OUTPUT_DIR / 'pipeline_real_results.png').resolve()}")
